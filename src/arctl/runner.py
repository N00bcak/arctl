...
def _research_prompt(task: LocatedTask, manifest: EvaluatorManifest) -> str:
    ...
    return (
        ...
        "Prefer material mechanisms. Treat any parameter-only or narrow local refinement "
        "including a coefficient, threshold, search depth or width, horizon, boundary, or "
        "feature variant as narrow. Propose one only when cited accumulated evidence has "
        "genuinely narrowed the uncertainty to that choice; explain why a broader structural "
        "repair is not presently better, and use a deliberate public sweep when applicable. "
        "Respect the candidate review contract and editable paths, do not commit, "
...
def _run_planning(
    ...
    *,
    ...
    prompt = (
        ...
        "never invent a weak candidate merely to continue. Untested or inconclusive "
        "evidence remains unresolved: it cannot support or contradict performance and "
        "cannot by itself exhaust a performance direction. For every parameter-only or "
        "narrow local refinement—including a coefficient, threshold, search depth or width, horizon, "
        "boundary, or feature variant—cite the prior experiment being refined, explain why "
        "the accumulated evidence justifies that specificity, and explain why a broader "
        "structural repair is not presently better. In each direction's feasibility and "
        "expected value, and again in the selection rationale, compare expected information "
        "or improvement against implementation size, runtime cost, and human audit burden. "
        "After comparing all "
        "directions, select the best request or set selection to null when all are "
        "exhausted. Use a deliberate public sweep for a narrow refinement when applicable. "
        "Do not edit, commit, or run hidden evaluation. Return only the required planning JSON."
        "\n\n" + json.dumps(packet, sort_keys=True, separators=(",", ":"))
    )
...
def _run_implementation(
    ...
    *,
    ...
    prompt = (
        ...
        "be established, do not substitute an easier idea: report infeasible. Before "
